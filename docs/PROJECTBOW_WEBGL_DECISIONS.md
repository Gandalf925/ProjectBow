...
- Known limitation: GitHub repository secrets are commonly repository-scoped. The first ProjectBow workflow run determines whether these secrets are already available to this repository; a missing-secret result is configuration evidence, not a code failure.
...
## PB-D-006 — Controlled initial CI trigger

- Date: 2026-07-27
- Status: Current
- Decision: The initial workflow runs from the same-repository Draft PR because GitHub only permits `workflow_dispatch` when the workflow file already exists on the default branch. The workflow does not run on ordinary branch pushes. After the workflow reaches `main`, future runs remain manual through `workflow_dispatch`.
- Safety: A secret-presence step runs before Unity activation. Missing secrets stop the job before the Unity builder starts.
